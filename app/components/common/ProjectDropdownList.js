...
class ProjectDropdownList extends Component {
    constructor(props) {
        super(props);
        this.state = { projects: [], defaultValue: this.props.defaultValue ? this.props.defaultValue._id : '*' };
    }
    loadDataFromServer() {
        let _this = this;
        fetch(url, {
    ...
    onDataChange(value) {
        ...
        if (!_project)
            _project = { _id: '*' };
        this.setState({ defaultValue: _project._id });
        this.props.onChange(_project);
    }
    render() {
        const { defaultValue } = this.state;
        return (
            <select className="form-control" onChange={(e) => this.onDataChange(e.target.value)} value={defaultValue}>
                <option value="*">{"*"}</option>
                {
